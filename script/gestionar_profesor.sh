...
gestionar_vms_pool() {
    ...
    echo "   2. Mover al pool del administrador"
    ...
    case "$OPCION_VMS" in
        1) for VMID in $VMS; do
               ...
               # Esperar a que la VM esté completamente destruida
               local INTENTOS=0
        ...
        2) for VMID in $VMS; do
               pvesh set /pools/pool_${USUARIO} --del-vms $VMID 2>/dev/null \
                   && echo "   - VM $VMID movida" \
                   || echo "   AVISO: No se pudo mover VM $VMID"
           done ;;
